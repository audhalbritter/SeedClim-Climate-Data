...
vegComp %>%
  ...
  facet_grid(precipLevel ~ tempLevel) +
...
soilTemp %>%
  ...
  filter(Treatment %in% c("GF", "GB", "FB", "FGB", "C"), weather %in% c("sunny", "cloudy"), between(date, ymd("2015-07-01"), ymd("2015-10-01"))) %>% 
...
soilTemp %>%
  ...
  stat_summary(fun.y = mean, geom = "line", size = 1) +
  facet_grid(precipLevel ~ tempLevel) +
  scale_color_manual(values = cbPalette[c(1, 3, 4, 8, 7)]) +
